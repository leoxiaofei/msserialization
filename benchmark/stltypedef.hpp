...
public:
    ...
    {
        // Helper lambda for comparing nullable pointers
        auto cmp = [](const auto& a, const auto& b) {
            if (a && b) return *a == *b;
            return !a && !b;
        };

        return cmp(pI, rhs.pI)
            && cmp(pT1, rhs.pT1)
            && cmp(pT2, rhs.pT2)
            && cmp(pT3, rhs.pT3)
            && (pBT1 && rhs.pBT1 ? pBT1->Equal(rhs.pBT1.get()) : !pBT1 && !rhs.pBT1)
            && (pBT2 && rhs.pBT2 ? pBT2->Equal(rhs.pBT2.get()) : !pBT2 && !rhs.pBT2)
            && (pBT3 && rhs.pBT3 ? pBT3->Equal(rhs.pBT3.get()) : !pBT3 && !rhs.pBT3);
    }
